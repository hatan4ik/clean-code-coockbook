...
func (r *PostgresRepository) Save(ctx context.Context, u domain.User) error {
	const query = `INSERT INTO users (id, email, username, created_at) VALUES ($1, $2, $3, $4)`
...
func (r *PostgresRepository) GetByEmail(ctx context.Context, email string) (*domain.User, error) {
	const query = `SELECT id, email, username, created_at FROM users WHERE email = $1`

	var u domain.User
	err := r.db.QueryRowContext(ctx, query, email).Scan(&u.ID, &u.Email, &u.Username, &u.CreatedAt)
	if err != nil {
